...
- 命中已知恶意附件哈希时，直接判定为恶意；
- 无附件时，附件结果为 `applicable=false`，不参与权重计算，但视为该模块检查完成；
- 加权后保留风险等级下限：任一可用模块分数 ≥70，总分至少 70；否则任一模块
  分数 ≥35，总分至少 35。零分未命中不能抵消其他模块的风险，代价是单模块误报
  也可能提升最终等级，后续应根据验证集校准；
- 附件危险类型同时依据扩展名和文件签名判断；未配置哈希查询、查询失败或未收录时
  返回 `partial`，保留本地风险分数，并将检测不完整状态传递到聚合结果；
